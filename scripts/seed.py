...
# Allow running directly: `python scripts/seed.py` from anywhere.
# (Running `python -m app.db.seed` is no longer needed — this adds the
#  project root to sys.path so `from app...` imports resolve.)
...
async def create_super_admin(
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not super_admin_role:
            logger.error("'Super Admin' role not found! Run 'python scripts/seed.py rbac' first.")
            return
